...
from utils.libs import strings
...
class U2NET:
    ...
    def __init__(self, name="u2net"):
        ...
        from utils.libs.u2net import U2NET as U2NET_DEEP
        from utils.libs.u2net import U2NETP as U2NETP_DEEP
        ...
        try:
            if self.torch.cuda.is_available():
                tmp_path = os.getenv('HOME') + '/Desktop/otesbro/otesbro/otes/utils/'
                net.load_state_dict(self.torch.load(os.path.join(tmp_path, "models", name, name + '.pth')))
                net.cuda()
            else:
...
class BasNet:
    ...
    def __init__(self, name="basnet"):
        import torch
        from torch.autograd import Variable
        from utils.libs.basnet import BASNet as BASNet_DEEP
